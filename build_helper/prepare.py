...
def prepare(configs: dict[str, dict[str, Any]]) -> None:
    ...
    global_files_path = os.path.join(paths.openwrt_k, "files")
...
def prepare_cfg(config: dict[str, Any],
    ...
    if clash_arch and openwrt.get_package_config("luci-app-openclash") == "y":
        ...
        #dl_tasks.append(dl2(f"https://raw.githubusercontent.com/vernesong/OpenClash/refs/heads/core/dev/smart/clash-{clash_arch}.tar.gz", os.path.join(tmpdir.name, "clash.tar.gz")))
    ...
    with open(os.path.join(openwrt.files, "etc", "openwrt_info"), "w", encoding="utf-8") as f:
        content = ""
        ...
        f.write(content)
    logger.debug("openwrt_info: %s", content)
